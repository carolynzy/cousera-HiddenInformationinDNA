...
for kmer in kmer_list:
	...
	for i in range(0,k):
		...
		if kmer[i] == 'T':
			count_t[i]+=1
	for i in range(0,k):
		profile_a[i]=count_a[i]
		profile_c[i]=count_c[i]
		profile_g[i]=count_g[i]
		profile_t[i]=count_t[i]
	...
	for i in range(3,t+2):
		...
		for j in range(0,k):
			...
			if motif_i[j] == 'T':
				count_t[j]+=1
		for j in range(0,k):
			profile_a[j]=count_a[j]/(i-1)
			profile_c[j]=count_c[j]/(i-1)
			profile_g[j]=count_g[j]/(i-1)
			profile_t[j]=count_t[j]/(i-1)
